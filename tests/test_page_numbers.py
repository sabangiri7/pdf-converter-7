...
def test_service_ok(sample_pdf_2p, sample_pdf_3p, tmp_path):
    out = run([sample_pdf_2p], tmp_path, position="bottom-center", start=1, format="n")
    assert out.is_file() and len(PdfReader(str(out)).pages) == 2
...
def test_get_page(client):
    r = client.get("/t/page_numbers")
    assert r.status_code == 200
    assert 'Add Page Numbers'.encode() in r.data


def test_route_roundtrip(client, sample_pdf_2p, sample_pdf_3p):
    r = client.post("/t/page_numbers",
                    data={**_upload(sample_pdf_2p), "position": "bottom-center",
                          "start": "1", "format": "n"},
