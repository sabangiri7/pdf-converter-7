...
def test_service_ok(sample_pdf_2p, sample_pdf_3p, tmp_path):
    out = run([sample_pdf_2p], tmp_path, count=1, at="end")
...
def test_get_page(client):
    r = client.get("/t/add_blank_pages")
    assert r.status_code == 200
    assert 'Add Blank Pages'.encode() in r.data


def test_route_roundtrip(client, sample_pdf_2p, sample_pdf_3p):
    r = client.post("/t/add_blank_pages",
                    data={**_upload(sample_pdf_2p), "count": "1", "at": "end"},
