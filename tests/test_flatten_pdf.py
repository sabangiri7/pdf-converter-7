...
from app.services.flatten_pdf import run
...
def test_get_page(client):
    r = client.get("/t/flatten_pdf")
    assert r.status_code == 200
    assert 'Flatten PDF'.encode() in r.data


def test_route_roundtrip(client, sample_pdf_2p, sample_pdf_3p):
    r = client.post("/t/flatten_pdf", data=_upload(sample_pdf_2p),
