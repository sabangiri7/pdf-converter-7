...
from app.services.remove_annotations import run
...
def test_get_page(client):
    r = client.get("/t/remove_annotations")
    assert r.status_code == 200
    assert 'Remove Annotations'.encode() in r.data


def test_route_roundtrip(client, sample_pdf_2p, sample_pdf_3p):
    r = client.post("/t/remove_annotations", data=_upload(sample_pdf_2p),
