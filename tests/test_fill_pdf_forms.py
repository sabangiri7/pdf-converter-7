...
def test_service_ok(sample_pdf_2p, sample_pdf_3p, tmp_path):
    with pytest.raises(ToolError):
        run([sample_pdf_2p], tmp_path, fields="Name=Test")
...
def test_service_rejects_bad_input(tmp_path):
    ...
    with pytest.raises(ToolError):
        run([junk], tmp_path, fields="a=b")


def test_get_page(client):
    r = client.get("/t/fill_pdf_forms")
    assert r.status_code == 200
    assert 'Fill PDF Forms'.encode() in r.data


def test_route_roundtrip(client, sample_pdf_2p, sample_pdf_3p):
    r = client.post("/t/fill_pdf_forms",
                    data={**_upload(sample_pdf_2p), "fields": "Name=Test"},
